...
const FilesContentApi = ({
  ...
  const fileTypes = acceptFileTypes
    ? acceptFileTypes
    : ['jpg', 'jpeg', 'png', 'pdf']
  ...
  const solicitation =
    fileList && fileList.length !== 0
      ? fileList[0]?.solicitationId
      : solicitationId
  // const solicitation = fileList && fileList.length !== 0 ? fileList[0]?.solicitationId : solicitationId
  const [imgModal, setImgModal] = useState('')
  const initialStateFileDelete = {
    confirm: '',
    file: '',
    fileId: '',
    openDialog: false
  }
  const [contentFileDelete, setContentFileDelete] = useState(
    initialStateFileDelete
  )
    ...
    if (get_url_extension(file.fileName) == 'pdf') {
      return (
        <Tooltip title='Visualizar PDF'>
          <IconButton>
            <VisibilityIcon
              onClick={() => showPdf(file.fileId, file.fileLink, file.fileName)}
            />
          </IconButton>
        </Tooltip>
      )
    ...
    if (fileTypes.includes(get_url_extension(file.fileName))) {
      return (
        <Tooltip title='Visualizar Imagem'>
          <IconButton>
            <VisibilityIcon
              onClick={() => showImg(file.fileId, file.fileLink, file.fileName)}
            />
          </IconButton>
        </Tooltip>
      )
  ...
  const showImg = (id, fileLink, fileName) => {
    ...
    setLoadChange(true)
    api.http
      .get(`${api.addressApiFile(id)}`, { responseType: 'blob' })
  ...
  const SendFiles = (e) => {
    ...
    if (e.target.files[0].size > maxSizeUpload) {
      alert(
        `O Arquivo é muito grande para o destino. Tamanho Maximo: ${
                  ...
                  <TableCell align=''>
                    <Tooltip title='Baixar Arquivo'>
                      <IconButton>
                        <FileDownloadIcon
                          onClick={() =>
                            handleGetDocumentApi(file.fileId, file.originName)
                          }
                        />
                      </IconButton>
                    </Tooltip>
        ...
        {editable && (
          <Fragment>
            ...
            <label htmlFor='raised-button-file'>
              <Tooltip
                title={`Arquivos até ${maxFileSizeMb ? maxFileSizeMb : 10} mb`}
              >
                <Button
                  variant='raised'
                  component='span'
                  sx={{ display: 'flex', color: '#6a6969' }}
                >
