...
const FilesUpload = ({
  ...
  const onTemplateSelect = (e) => {
    ...
    if (totalSizeToAdd > maxSizeUpload) {
      toast.current.show({
        severity: 'error',
        summary: 'Atenção:',
        detail: `Os Arquivos Selecionados Excedem o Tamanho Total de Arquivos. (${formatBytes(
          maxSizeUpload
        )})`
      })
      return
    ...
    filesUpload.forEach((file) => {
      ...
      if (!list.find(isExistFileUpload)) {
        ...
        toast.current.show({
          severity: 'warn',
          summary: 'Aviso:',
          detail: `${file.name} - Arquivo já Adicionado!`
        })
      }
    })
  ...
  const headerTemplate = (options) => {
    ...
    const formatedValue = totalSize > 0 ? formatBytes(totalSize) : '0 B'
      ...
      >
        ...
        <ProgressBar
          ...
          displayValueTemplate={() =>
            `${formatedValue} / ${formatBytes(maxSizeUpload)}`
          }
